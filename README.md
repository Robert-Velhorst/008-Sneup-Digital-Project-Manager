...
The local release line currently builds `Sneup-Setup-2.3.74.exe`. The generated installer is unsigned unless a publisher certificate is configured in the release environment. Treat unsigned installers as internal test artifacts.
...
For manual Trello-result reconciliation, `npm run verify:trello-reconciliation` uses `SNEUP_RECONCILIATION_VERIFICATION_MONGO_URI`. Set it to a new, empty database named `sneup_reconciliation_verification_<unique-suffix>` (at most 63 bytes), for example on loopback MongoDB. It refuses nonempty targets, claims exclusive ownership, and removes only its verified temporary database. CI runs it against its disposable MongoDB service. It tests competing reviews, interrupted writes, lost acknowledgements, legacy records, approval binding, workspace isolation, internal-effect recovery, duplicate prevention, early responses, and bounded worker retries without calling Trello.

**Recovering an uncertain Trello result:** open **Approvals**, find the action, and choose **Reconcile result** only after checking the actual Trello activity/state. This records an internal outcome, not another provider request. An accepted reconciliation decision retains its original result, evidence, note, and actor. After an interrupted save, reopen the action to resume those recorded values; contradictory reviews are rejected. Failed requests keep entered evidence visible, and a failed refresh is distinguished from a failed save. Audits belong to the action's workspace.
...
**Workspace ownership of linked evidence:** starting with 2.3.72, workspace-qualified reference loading is applied across the main board/card routes, analytics and team/context workflows, conversational and priority views, operational-ledger services, and model-level query helpers. Nested comment authors, assigned cards and their boards are scoped too. A foreign-workspace, unscoped legacy, or missing singular reference is hidden while its authorized local root remains visible; valid local links still populate. Reads do not repair stored links or authorize Trello writes.

The regression was reproduced through authenticated HTTP with deliberately malformed stored links. Verification confirms valid local board/card/member/comment references still work, foreign linked records stay hidden, and reading a local card does not run NLP writes against a foreign comment. This does not establish that a normal user can create those links through a public write endpoint. It is not a guarantee covering every aggregation or denormalized query, and it does not repair historical records. Scoped population queries use a five-second MongoDB execution limit; latest-board-health population follows its existing configurable query deadline. Limits are per query, not an end-to-end request timeout. No production-scale speed or memory improvement is claimed.

`npm run verify:ledger-reference-isolation` verifies these boundaries on a new, empty MongoDB database named `sneup_ledger_references_<unique-suffix>` (at most 63 bytes). It claims exclusive ownership before loading models and removes only its verified temporary database. It exercises authenticated board/card HTTP, local and malformed foreign/unscoped/dangling references, nested members/comments, NLP write isolation, priorities, work graph dependencies, daily briefs, and timeout/partial-failure handling without real provider calls. CI includes this profile. For local MongoDB in PowerShell:
