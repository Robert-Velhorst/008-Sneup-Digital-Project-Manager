...
Session-list and confirmation results are scoped to their original workspace, session generation, and dialog. Closing a submitted self-revocation confirmation does not skip sign-out. Revocation of another session can refresh its list without signing out the caller, and a failed post-revocation refresh is not mislabeled as a failed revocation.

The shared request path also signs out after a protected request returns HTTP 401 with `X-Sneup-Authentication: required`, emitted by Sneup credential rejection only. It checks the credential actually sent and the current session before clearing records and permissions. Provider 401 responses without that header, role 403 responses, invitation failures, and temporary 503 failures do not invalidate the current session. An unavailable database credential store returns 503 rather than falsely declaring the token invalid. Valid environment keys and intentional no-credential local access retain their existing behavior.

After a session changes, old protected responses and delayed JSON bodies are discarded; they cannot restore the prior workspace's data. Responses for a newer session are not invalidated by a late rejection of its predecessor. A committed invitation acceptance is retained across an intervening sign-out, but not across another workspace/session transition. Signed-out requests are blocked locally except invitation acceptance. No background polling or mutation replay is added. An idle window detects remote revocation only on its next request; already-authorized server work is not canceled universally. Same-session workspace transitions in other mutation/detail flows remain under review.
...
The first command is read-only JSON evidence: it reports each collection's records missing `workspaceId`, the target workspace identifier, bounded concurrency, and aggregate-only duplicate counts that would conflict with workspace-scoped policy, job-control, feature-flag, or Trello-entity indexes. It never returns policy conditions, user data, credentials, affected record identifiers, or record content. The `--apply` command refuses any detected duplicate group before it creates the default workspace, backfills anything, or changes an index. Once clean, it attaches only legacy records where `workspaceId` is absent or `null`; it does not overwrite a record already assigned to another workspace. It then replaces legacy global `trelloId` uniqueness on boards, lists, cards, comments, and members with `workspaceId + trelloId`, allowing intentionally overlapping Trello sources without weakening Sneup workspace isolation.
