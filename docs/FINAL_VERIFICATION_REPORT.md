...
## 2.3.62 recommendation review context and verifier ownership (2026-09-07)

- Recommendation actions and payload-review forms capture workspace/session/generation ownership. Payload context lookups cannot open stale forms, detached controls cannot submit, and a shared per-context pending set excludes overlapping actions and payload saves on one recommendation. Exact expected revisions and backend approval authority remain unchanged. Workspace reset clears payload-review and operating-ledger dialogs; modal dismissal advances an epoch so delayed recommendation/drilldown results cannot reopen them.
- Approval controls bind only inside their owned view containers, and operating-ledger recommendation controls bind only inside that modal. An acknowledged decision or payload save is reported separately from a failed subsequent ledger refresh. The failed refresh invalidates the Approvals cache so reopening performs a new read. No mutation is automatically replayed.
- Initial deferred-response tests reproduced nine regressions. Independent source review then identified model initialization before verifier ownership, dismissed-dialog recovery, and cached failed-refresh recovery; all were fixed with permanent tests. Final independent source review reported no remaining actionable finding in this scoped diff. It did not execute tests or certify the rest of the application.
- Final local `npm run check:ci` passed lint, 180 route-authorization contracts, 154 suites/1,411 tests, and all five recommendation scenarios. An intermediate full-suite failure was limited to four dashboard-reset tests whose callback fixture needed the new context callback; the corrected complete suite passed. The fresh dependency audit reported zero known vulnerabilities. The release version was bumped afterward without changing dependencies or runtime code.
- `npm run verify:review-concurrency` passed against owned, empty loopback MongoDB database `sneup_review_concurrency_verification_573ae1c576fb4f5c` in 2,982.4 ms. It checked competing decisions, payload/approval races, stale and terminal queue rejection, exact active approval binding, zero orphan approvals, and zero Trello action attempts. No provider writes occurred. Cleanup removed the owned database and disconnected. An earlier overlong disposable name was rejected by MongoDB; the verifier now rejects names over 63 bytes before connection, inspects emptiness before importing models, and disconnects even when verification/cleanup fails. Dedicated tests cover nonempty database refusal and import ordering.
- Final browser verification used actual UI assets with hash `fe2bc9fc99e4ebd3` at `http://127.0.0.1:3299` and synthetic responses. A payload save in workspace A was held for 15 seconds, its modal closed, and workspace B selected. After A's response, B remained selected, A's record was absent, and no modal reopened or obsolete ledger refresh ran. A normal B save displayed the fresh-approval notice and reopening the form showed the saved B text. Page identity, meaningful content, screenshot inspection, zero warning/error console entries, and no horizontal document overflow passed at 2,560 CSS pixels. No mobile or clean-machine visual acceptance is claimed. The fixture and tab were stopped afterward.
- The browser fixture made only two synthetic in-memory payload writes and initialized no MongoDB, workers, real provider accounts, or tunnel. Real MongoDB evidence came from the separate service-level verifier, not that browser journey. No backend model, route, provider authority, dependency, or polling change was made. Duplicate browser work is suppressed, but no comparative speed or memory improvement is claimed.
- The published commit's CI is authoritative for the Windows installer build and packaged-launch result; no independent local 2.3.62 installer build was performed. Other mutation/detail flows, complete application-wide context isolation, live provider/ngrok/HAI acceptance, signed clean-machine Windows installation, and the broader production objective remain open. Already accepted server-side work is not universally canceled.

## 2.3.61 confirmed session rejection and in-flight response handling (2026-09-07)

- The shared request path now clears cached workspace data and permissions when Sneup confirms rejection of the current credential through HTTP 401 and `X-Sneup-Authentication: required`. The existing non-secret signed-out marker prevents automatic local-owner fallback and repeated protected requests. Ordinary provider errors, role denials, explicit alternative credentials, public invitation failures, and replacement sessions do not trigger this reset. An unavailable credential database returns 503 without the rejection header; valid environment-key access retains its existing behavior.
- Both successful and failed delayed JSON bodies check their original session. Old responses are discarded without automatically replaying possibly committed changes. A successful single-use invitation can establish its returned session across one intervening sign-out, while a newer identity remains protected. Pending workspace exports use an abort signal, check context after file-picker/header/writable/blob waits, and abort streamed output on context reset. Normal file and blob exports remain functional. Rollout-history success and error handlers preserve a newer modal or sign-out controls.
- Initial tests reproduced missing authentication signals and stale-session handling. Independent review additionally reproduced delayed-error, late-export, and rollout-history recovery regressions; all three were fixed with permanent tests. Final independent review found no remaining concrete finding in scope and passed all 57 session-transition tests, including ordinary and canceled exports, delayed errors, and composed invitation acceptance.
- Local verification passed `npm run check:ci`: lint, 180 route-authorization contracts, 152 suites/1,390 tests, and five recommendation scenarios. The dependency audit reported zero known vulnerabilities. An intermediate full-suite failure came from seven path-compatibility tests whose response mock lacked the new header method; the mock was corrected and the full suite rerun.
- `npm run verify:hai-http` passed 42 real HTTP checks in an owned empty loopback MongoDB database, including expired and disabled-user sessions, revocation elsewhere, rejection-header semantics, role denials, workspace isolation, report downloads, and HAI read/propose-only authority. The verifier created no approvals or provider action attempts, made no provider writes, and removed its temporary database.
- Final browser verification used actual UI assets with hash `597fd0dfc7622869` and synthetic responses at `http://127.0.0.1:3299`. Joining workspace B, opening administration, then requesting Forecasts produced a confirmed rejection. Old user and dashboard data disappeared, the selector became disabled, and the signed-out dialog appeared. Done followed by Refresh remained signed out without another protected request; explicit **Use local access** restored workspace A and its forecast. Page identity, meaningful rendering, absence of framework overlays, zero warning/error console entries, and screenshot checks passed at 2,560 CSS pixels with no horizontal document overflow. The fixture and tabs were stopped afterward. No mobile, native file-picker, or clean-machine visual acceptance is claimed.
- No database model, dependency version, provider-write authority, or background polling was added. Rejected-session requests are suppressed and abandoned exports are canceled; no comparative speed or memory improvement is claimed. The published commit's CI is the authoritative Windows installer/packaged-launch result; no independent local 2.3.61 installer build was performed during this change.
- Remaining work includes other mutation/detail flows and same-session workspace transitions, recovery of every interrupted invitation flow, live provider/ngrok/HAI acceptance, signed clean-machine Windows installation, and the full production objective. An idle window detects remote revocation on its next protected request, not instantaneously. Already-authorized server-side work is not universally canceled.

## 2.3.60 session revocation and local credential boundaries (2026-09-07)

- Session-list reads, rendered session actions, and revocation confirmations check their original workspace/session/generation and dialog. Duplicate confirmation clicks are ignored. A confirmed revocation is distinguished from a failed subsequent list refresh; incomplete responses are explicitly unconfirmed. The API returns `currentSessionRevoked` only for the exact authenticated database session being revoked.
- Confirmed self-revocation clears cached dashboard/administration records and permissions even after the waiting dialog closes. The current raw credential is replaced with a non-secret signed-out marker in memory and session storage. Ordinary API requests are blocked while marked; only the exact invitation-acceptance POST remains available. Refresh/startup keep the sign-out notice usable even when its optional localization module fails. Loopback pages offer explicit **Use local access** re-entry, subject to the backend's authentication policy. Storage failure does not authorize local access.
- Explicit invalid, revoked, expired, or malformed supplied credentials no longer fall back to localhost owner access. Database sessions and database API tokens also remain workspace-bound on localhost; session/current-context metadata and catalog visibility reflect that boundary. Intentional no-credential local mode and environment-key service behavior remain supported.
- Initial regressions reproduced 12 browser failures and three credential-fallback failures. Independent review identified the automatic local re-entry gap and an insufficient foreign-workspace test. A stronger real HTTP test using a foreign owner reproduced an incorrect 200 response and actual revocation in a disposable database before the workspace-bound correction. The final review found no remaining actionable issue in scope, with 36 UI, 12 selected security, and 15 additional synthetic checks passed.
- Final focused UI verification passed 48 tests across session transitions and workspace rendering/localization. The full local `npm run check:ci` passed lint, all 180 route-authorization contracts, 152 suites/1,368 tests, and all five recommendation scenarios. The dependency audit reported zero known vulnerabilities. The final dialog change only moves the local-access button into the existing actions row; the published-commit CI reruns the suite on the packaged source.
- The extended `npm run verify:hai-http` passed 40 real HTTP checks against a dedicated empty loopback MongoDB database. It verifies session identity, owner-scoped catalog/context, non-administrator denial, privileged foreign-session and foreign-API-token denial, revocation persistence, exact before/after high-risk audit records, and rejection of both revoked tokens with authentication required and with local mode enabled. It also retains HAI read/propose-only, report download, and invalid-relationship checks. No provider writes occurred, and the verifier removed only its owned temporary database and closed its HTTP listener.
- Browser verification used actual UI scripts with synthetic invitation/session responses at `http://127.0.0.1:3299`. The final behavioral run used fresh asset hash `56d7758ff75bffac`; a prior stale-cache run was excluded. After confirming self-revocation and closing the pending dialog, the 15-second response cleared the workspace and reopened the signed-out notice. Closing that notice and clicking Refresh kept the selector disabled and offered explicit local re-entry. Clicking **Use local access** restored the synthetic workspace and closed the modal. No warning/error console entries or framework overlay appeared. At 2,560 CSS pixels the document had no horizontal overflow; a clipped screenshot verified the notice. No mobile or clean-machine visual acceptance is claimed.
- Browser responses were synthetic and did not touch accounts, MongoDB, workers, providers, or tunnels. The separate real HTTP verifier used only disposable seeded identities. No database model, dependency version, provider-write authority, or background polling was added. Blocking signed-out requests avoids unnecessary API/database work; no comparative performance improvement is claimed. Check the published commit's CI for the unsigned Windows installer build and packaged launch probe; this version was not independently built or benchmarked locally.
- Remaining work includes general UI handling of sessions revoked by another browser, other mutation/detail forms, interrupted invitation recovery, live provider/ngrok/HAI acceptance, signed clean-machine Windows installation, and the broader production objective. Already-authorized in-flight work is not universally canceled. The overall goal and cross-user isolation remain partial.

## 2.3.59 invitation and deletion session transitions (2026-09-07)

- Invitation acceptance and confirmed deletion use the same in-memory context reset as workspace switching. A new token in the same workspace also clears cached administration/dashboard records, permission context, loaded-view flags, and obsolete modal content. Browser-storage failures do not turn an acknowledged server result into a failed acceptance/deletion. An incomplete deletion receipt is explicitly unconfirmed.
- Identity ownership captures workspace, token, and epoch independently of form-opening request keys. Form ownership gates submission and stale error presentation; completed single-use acceptance and completed deletion are retained for an unchanged identity even when the form has closed. Reopening deletion does not suppress its pending cleanup. Newer identities remain protected, and accepted-session reload notices cannot overwrite newer dialogs.
- The initial eight regressions all failed against the baseline. The final focused run passed 28 tests across the new 16-case session-transition suite and existing workspace-view suite. The new tests compose the actual invitation controller with the extracted application functions and deferred synthetic HTTP/storage responses. Independent review found and prompted fixes for dismissal and reopening regressions, then passed 28 focused checks and six additional workspace/token/epoch probes with no remaining finding in the narrow correction.
- Browser verification used the actual final UI scripts at `http://127.0.0.1:3299` with an isolated synthetic fixture. Submitting an invitation, closing the form, and receiving the delayed response 15 seconds later selected workspace B and reloaded the overview with the modal closed. Page identity, meaningful content, no framework overlay, and zero warning/error console entries were confirmed. At 2,560 CSS pixels the document width was 2,529, without horizontal overflow. A screenshot was captured, but host scaling/cropping limits visual completeness. No mobile verification is claimed for this increment. The temporary tab and server were closed.
- The fixture returned an in-memory invitation token, not a real account session, and never initialized MongoDB, workers, provider writes, or a tunnel. Deletion behavior was tested with mocked receipts/storage, not a real deletion. No backend service, database model, route, dependency version, or provider authority changed.
- The full dependency audit reported zero known vulnerabilities. Live provider/ngrok/HAI acceptance, signed clean-machine Windows installation, other mutation/detail forms, session revocation, and recovery of every interrupted invitation flow remain unverified. The broader goal and cross-user isolation remain partial.
- The final local `npm run check:ci` passed lint, all 180 route-authorization contracts, 152 suites/1,342 tests, and all five recommendation scenarios. README relative file links resolve, and `git diff --check` passed. GitHub's paginated open-pull-request inventory was empty before publication. Check the published commit's CI run for Linux quality, MongoDB integration, and unsigned Windows installer build/launch results; the 2.3.59 installer was not independently built or benchmarked locally in this increment.

## 2.3.58 selector refresh and forecast form ownership (2026-09-07)

Current local verification record; check remote CI against the published commit.

- The workspace selector now reloads alongside the active view after its workspace module has first loaded. It reads only the bounded catalog, without triggering hidden integrity, retention, policy, or membership scans. Administration uses the same catalog reader, with shared ownership of records, loading state, and errors. Locked/demo administration invalidates older catalog work. A current workspace outside the bounded page remains selected, and catalog failures remain retryable through Refresh.
- Three forecast forms capture their opening workspace/session/generation and check both context and modal ownership before submission and completion. They suppress duplicate submissions, do not close or replace newer modals, and are removed during workspace adoption. Scenario responses coordinate with ordinary forecast GET ownership. Capacity/mapping saves acknowledged by the server are distinguished from failed subsequent refreshes, with English/Dutch feedback. These changes do not cancel or undo an accepted server request.
- The initial 24 form regressions reproduced 21 failures, with the three ordinary success cases already passing. The final focused group passes 71 tests across form ownership, selector behavior, administration ownership, and dashboard reset. Composed tests acknowledge a save, hold the actual forecast reader's data and module promises, change workspace, and verify that no stale forecast, rendering, modal closure, or notice follows. Other coverage includes session/generation changes, duplicate submission, dismissed forms, same-context forecast supersession, failed refresh after successful save, and selected-workspace fallback.
- Independent review reproduced an introduced cross-loader catalog race, which was fixed by routing both writers through the shared reader. Both late-success and late-failure regressions are retained. The narrow re-review passed 33 focused tests plus six additional in-memory reverse-order and locked/demo cases, with no remaining findings in that correction. Other modal and invitation/session flows were explicitly outside that review.
- Final `npm run check:ci` passed lint, all 180 route authorization contracts, 151 suites/1,326 tests, and all five recommendation scenarios on version 2.3.58. The full dependency audit reported zero known vulnerabilities. The paginated GitHub inventory contained no open pull requests before publication.
- The unsigned Windows 2.3.58 installer built successfully (109,510,927 bytes). All three changed packaged UI files match source byte for byte. The five-second packaged demo probe passed health, nine redacted diagnostics, HAI `never_direct`, normal close, main exit code zero, settled process inventory, no remaining processes, and port release. Four processes sampled 374 MB working set, 333.6 MB private memory, and 1.406 cumulative CPU seconds. This short shared-machine sample does not prove a performance improvement, signed release, or clean-machine installation acceptance.
- Browser verification used the actual scripts at `http://127.0.0.1:3299` against a synthetic loopback fixture. Its sole accepted POST returned a delayed, in-memory forecast scenario; all other non-GET requests were rejected. No database initialization, workers, real accounts, provider actions, or tunnels ran. After first opening Workspaces, repeated A/B switching from Reports worked without returning to administration. On the final code, a scenario started in A was canceled, B was selected, and A's response arrived 15 seconds later. B's normal forecast remained, the modal stayed closed, and both selector options remained available. Narrow-width switching back to A and reopening/canceling its scenario form also worked.
- Page identity, nonblank content, no framework error overlay, interaction behavior, and zero warning/error console entries were checked. DOM measurements found no horizontal document overflow at 2,560 and 449 CSS pixels. Screenshots were captured, but host scaling/cropping limits their completeness; they do not establish pixel-perfect or exact-device visual acceptance. The size override was reset and temporary tabs/server were closed.
- No database model, backend service, route, dependency, or provider authority changed. Other mutation/detail forms, invitation/session transitions, live provider/ngrok/HAI acceptance, signed clean-machine installation, and the broader production goal remain open. Cross-user isolation remains partial; passing these bounded checks is not application-wide certification.
