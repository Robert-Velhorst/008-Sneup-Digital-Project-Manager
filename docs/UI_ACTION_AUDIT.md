...
- The 2.3.28 in-app Browser proved `workspaceView.js` is absent on Overview and loads exactly once after Workspaces opens. Export, deletion, and invitation mutations remained disabled in explicit demo mode; English/Dutch responsive layouts had no document overflow; `/api/v1/security/context` returned a correlated `200` with demo viewer identity and provider writes disabled; the browser reported zero current warnings/errors.
- The 2.3.27 in-app Browser proved `workspaceView.js` is absent on Overview and loads exactly once with the shared fingerprint after Workspaces opens. Demo invitation/export/deletion controls remained disabled, English/Dutch rendering and desktop/mobile containment passed, an invitation query was removed before display without submission, and both tabs reported zero current console warnings/errors. Seeded DOM tests cover protected create/resend/revoke/accept outcomes without fabricating live identity state.
- The 2.3.26 in-app Browser proved `setupView.js` is absent on Overview and loads exactly once with the shared fingerprint after **Set up** opens. All eight diagnostics rendered in English and Dutch, refresh completed, evidence remained escaped, desktop and responsive layouts had no horizontal overflow, and the console reported zero current warnings/errors. Seeded DOM tests cover desktop commit, retry, stale-response, and duplicate-action behavior that browser demo mode cannot fabricate.
...
- For 2.3.25, the in-app Browser proved `approvalView.js` is absent on Overview and loads exactly once with the app fingerprint after Approvals opens. English/Dutch approval-ledger rendering, read-only demo controls, zero fabricated policy forms, no horizontal overflow, and zero current console errors passed. Seeded DOM tests cover create/edit values, daily schedules, activation confirmation, external-test confirmation, duplicate-action locks, retries, and truthful post-commit refresh failures.
- For 2.3.26, the in-app Browser proved `setupView.js` is absent on Overview and loads exactly once with fingerprint `0f4df0e31230dfab` after **Set up** opens. Eight diagnostics, refresh, English/Dutch rendering, desktop and constrained responsive containment, and zero current console warnings/errors passed. Packaged Windows verification separately confirmed eight diagnostics, HAI `never_direct`, normal close, and port release.
- For 2.3.27, the in-app Browser proved `workspaceView.js` is absent on Overview and loads exactly once with the current shared fingerprint after Workspaces opens. English/Dutch read-only controls, disabled mutations, invitation-query removal, desktop/mobile containment, and zero current console warnings/errors passed. Packaged Windows verification separately confirmed version 2.3.27, eight diagnostics, HAI `never_direct`, normal close, port release, and byte-identical changed runtime files.
