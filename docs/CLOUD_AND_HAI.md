...
1. Windows 11 desktop through the current `Sneup-Setup-2.3.32.exe` release target.
...
Sneup refuses to create public ingress when the ngrok token, API-key enforcement, or strong API key is absent. Concurrent startup calls share one in-progress listener. Graceful or partial-startup cleanup first cancels every loaded scheduler, then attempts ngrok, HTTP, and MongoDB cleanup even if one component fails. Tunnel-owned ephemeral URLs are removed and operator-provided public or webhook callback configuration is restored. A later ephemeral tunnel therefore receives a fresh callback instead of reusing a dead URL.
