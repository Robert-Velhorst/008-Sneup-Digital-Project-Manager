...
1. Windows 11 desktop through the current `Sneup-Setup-2.3.31.exe` release target.
...
Sneup refuses to create public ingress when the ngrok token, API-key enforcement, or strong API key is absent. Concurrent startup calls share one in-progress listener. On graceful shutdown it closes the tunnel before stopping the HTTP server and database connection, removes tunnel-owned ephemeral URLs, and restores any operator-provided public or webhook callback configuration. A later ephemeral tunnel therefore receives a fresh callback instead of reusing a dead URL.
