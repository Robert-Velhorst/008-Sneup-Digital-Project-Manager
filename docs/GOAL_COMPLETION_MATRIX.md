...
| 046 Cross-user isolation | Partial | Database credentials stay workspace-bound on localhost; privileged foreign session/API-token revocations are rejected in real HTTP/Mongo checks. Administration/report downloads, primary GET readers, selector, forecast forms, invitations, deletion, session list/revocation transitions, matching same-browser session invalidation, recommendation-evidence reads, and work-graph detail reads have bounded guards. Other forms/detail reads, idle cross-device revocation notification, and interrupted-onboarding recovery remain open. |
