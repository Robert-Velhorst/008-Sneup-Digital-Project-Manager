...
2.3.71 restricts populated ledger, daily-brief, health, findings, and work-graph dependency references to the selected workspace. The owned MongoDB and authenticated HTTP verifier covers malformed cross-workspace references while retaining legitimate local references. This hides bad references without repairing old records. Other app-wide relationship paths and nested data, scale/resource/failover evidence, live provider/ngrok/HAI, signed clean-machine Windows acceptance, and the complete production goal remain open.
