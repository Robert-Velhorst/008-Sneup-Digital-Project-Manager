...
2.3.64 makes manual Trello-result reconciliation version-checked and resumable using the original operator decision. It protects normal and unversioned legacy document saves, rejects historical/mismatched-approval attempts, scopes linked reads and audit evidence, and preserves provider-step evidence. The dashboard retains failed input, resumes locked recorded evidence, guards workspace/session ownership, and distinguishes saved results from failed refreshes. Real-Mongo checks include the actual executor behind a held synthetic provider boundary; browser checks use synthetic responses. Downstream crash recovery and late contradictory provider evidence, other UI contexts, live provider/ngrok/HAI acceptance, signed clean-machine Windows installation, and the full production/resource objective remain incomplete.
