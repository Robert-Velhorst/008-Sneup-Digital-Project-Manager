...
| 046 Cross-user isolation | Partial | Database credentials stay workspace-bound on localhost; privileged foreign session/API-token revocations are rejected in real HTTP/Mongo checks. Administration/report downloads, primary GET readers, selector, forecast forms, invitations, deletion, and session list/revocation transitions have bounded guards. Other forms, detail reads, general cross-browser revocation UI handling, and interrupted-onboarding recovery remain open. |
