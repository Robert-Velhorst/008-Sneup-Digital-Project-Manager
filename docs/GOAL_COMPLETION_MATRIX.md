...
2.3.74 closes notification-policy UI ownership gaps. Policy save/edit, activation, explicitly confirmed test delivery, and inline pause flows now reject stale submissions and suppress late notices/closures after workspace, session, modal, or row ownership changes. Eight new cases cover the previously reproduced failures. The local gate passed 160 Jest suites/1,589 tests, 180 route-authorization contracts, and five recommendation scenarios; both dependency audits were clean. Exact-commit hosted quality, Mongo/backup, and Windows packaged verification passed in [run 35808681311](https://github.com/Robert-Velhorst/008-Sneup-Digital-Project-Manager/actions/runs/35808681311). The backend approval and authorization boundaries are unchanged; other detail/forms, comprehensive source scanning, provider acceptance, and cross-device coordination remain open.

2.3.72 applies workspace-scoped reference hydration to primary board/card, context/team, NLP, conversational, priority, autopilot, intervention, analytics, performance, chat, job-observability, ledger, and model-helper paths. An owned MongoDB plus authenticated HTTP verifier passes 21 scenario groups over 28 read routes, including mixed foreign links, nested comments/members, unchanged foreign-comment NLP state, and legitimate local references. This hides malformed links without repairing records. Full aggregation/denormalized-path isolation, resource and failover measurement at production scale, live provider/ngrok/HAI acceptance, signed clean-machine Windows acceptance, and the complete production goal remain open.

The session follow-up broadcasts confirmed invalidation to matching tabs in the same browser using a SHA-256 fingerprint rather than the session credential. Focused UI tests cover credential rejection, cross-tab cache clearing, unrelated-session isolation, and completed workspace deletion. This does not notify idle sessions on separate devices. The full local gate passes 160 Jest suites/1,570 tests, 180 route-authorization contracts, and all five recommendation scenarios.

The recommendation-evidence and work-graph item detail reads now bind both success and error presentation to the initiating workspace, session, latest request, and modal content/epoch. Superseded or dismissed responses cannot replace a later view, and item identifiers are encoded in their API paths. Sixteen focused jsdom cases cover stale successes/failures, overlapping requests, and path encoding. The inbound worker-response binding and provider account-selection dialogs now apply the same ownership checks, discard removed-account results, and use the current account object after refresh; 20 focused cases cover those flows. Other detail/forms, idle cross-device invalidation, and interrupted onboarding remain open. The Codex Security Deep Scan did not start because its read-only workers require an unavailable managed filesystem permission profile; current repository-wide scan coverage is unverified.

2.3.73 closes one of the remaining connector-form mutation gaps. Account-selection and inbound worker-response forms now bind submission, asynchronous searches, refreshes, and completion notices to the initiating workspace/session, latest read, account, modal content, and epoch. A deferred-save regression failed before the fix because a stale completion closed a newer dialog. Eleven new tests cover the red failure, stale pre-submit, success ownership for both forms, and controller invalidation across workspace, session, account, and modal changes. Other already-open mutation/detail forms remain unaudited.

The 2.3.72 local gate passed lint, 180 route-authorization contracts (174 guarded, six explicitly public, no issues), 160 Jest suites/1,570 tests, and five recommendation scenarios. The owned-Mongo verifier reported zero real provider calls. Local tests do not establish live-account permissions, deployments, or a signed Windows release.

The 2.3.73 local gate passed lint, 180 route-authorization contracts (174 guarded, six explicitly public, no issues), 160 Jest suites/1,581 tests, and all five recommendation scenarios. Full and production-only dependency audits report zero vulnerabilities. This change made no provider calls or writes. Exact-commit hosted quality, Mongo/backup, and Windows package checks all passed in [run 35806582393](https://github.com/Robert-Velhorst/008-Sneup-Digital-Project-Manager/actions/runs/35806582393); no live provider, ngrok, or HAI acceptance is claimed.

The full dependency audit had found GHSA-2883-xcg3-v3hh in development/build-tool transitive `js-yaml` copies; the production dependency tree was clean. The lockfile now resolves patched 3.15.2 and 4.3.2 versions, and both full and production-only audits report zero vulnerabilities. This is dependency-advisory evidence, not a substitute for the current repository-wide scan, which remains unavailable.

2.3.70 connects backend section-level ledger errors to the actual Approvals UI: unavailable metrics/counts/lists no longer imply healthy zeroes, valid evidence remains accessible, and incomplete timelines are labeled. Summary validation and current-availability cache checks prevent malformed objects and delayed screen loads from hiding failures or blocking retries. Real-Mongo/versioned HTTP and a read-only browser fixture verify partial failure, whole-read failure, and recovery. These changes add no polling or provider authority. Other dashboard/detail contexts, complete nested-record validation, production-scale/resource and failover acceptance, live provider/ngrok/HAI, and signed clean-machine Windows acceptance remain open; the full goal is not complete.

2.3.69 fixes reconciliation queue starvation by completed history: MongoDB selects unresolved candidates before the display limit, and all related records are workspace-scoped with explicit query timeouts. A real-Mongo/authenticated-HTTP verifier reproduces the old omission behind 5,000 completed attempts, checks all seven eligible entries, and verifies failure propagation without provider calls. Only seven candidate identities entered display hydration in this fixture, compared with 5,008 rows in a full-history reference read. This is bounded synthetic evidence, not production-scale latency/memory acceptance. Health counts remain page-scoped; concurrent writes can shorten a page. Full production/resource/bug-fixing work, live provider/ngrok/HAI acceptance, and signed clean-machine Windows acceptance remain open.

2.3.68 extends the existing recovery path to newly recorded definitive Trello failures, using an explicit persisted failure marker. The same bounded worker completes failed recommendation/intervention state and one audit, without repeating the provider request or creating follow-ups. API failures retain distinct complete/pending codes; the dashboard refreshes failed state and preserves workspace/modal ownership. Real-Mongo before/after-write fault tests and synthetic browser interactions verify these paths. Ambiguous or unsaved provider results, historical records without plans/markers, attempt-creation interruptions, best-effort learning, process/failover and production-scale testing, live provider/ngrok/HAI acceptance, and signed clean-machine Windows acceptance remain incomplete. The full goal remains open.

2.3.67 extends internal recovery to new normal approved executions with a persisted successful Trello result. Stable attempt plans finish recommendation/intervention/follow-up/audit work without another provider call, retain original actor/time, preserve terminal intervention and manual-reconciliation ownership, and recover late worker responses. Bounded workspace retries rotate invalid references. Real-Mongo fault injection and the synthetic dashboard journey verify these paths. Unsaved or ambiguous provider results, historical records without plans, best-effort learning, real process/failover behavior, comparative performance at production scale, live provider/ngrok/HAI acceptance, and signed clean-machine Windows acceptance remain incomplete. The full production/resource/bug-fixing goal remains open.

2.3.66 extends durable internal-effect recovery to new worker responses. Exact intervention attachment is required before confirming a pending response; a fixed follow-up set, canonical receipt, and stable audit identities prevent retries from duplicating events or silently absorbing later work. Later Trello-reconciliation follow-ups have separate audited resolution. The existing worker retries bounded workspace batches, and the dashboard discloses pending internal work. Real MongoDB fault injection and synthetic browser interactions verify these paths, not live provider or process-kill/failover behavior. Historical records without plans, all normal provider-execution crash paths, late contradictory provider evidence, other UI contexts, live provider/ngrok/HAI acceptance, signed clean-machine Windows installation, and the full production/resource objective remain incomplete.

2.3.63 extends context ownership and duplicate suppression to decision snoozing/delegation, manual follow-up actions, outcome evaluation, and worker-response recording. Pending recommendation and ledger actions survive workspace round trips, while linked manual/worker follow-up actions share one guard. Worker controls use existing populated intervention eligibility, and the backend returns the actual follow-up result without response text. The follow-up MongoDB verifier adds exclusive ownership before model loading and checks the token before dropping its disposable database. Local, real-Mongo, and synthetic browser evidence covers these paths; other mutation/detail flows, cross-device conflict handling, provider/ngrok/HAI acceptance, signed clean-machine Windows installation, and the complete production objective remain open.

2.3.62 extends context ownership to recommendation decisions, payload review, and operating-ledger opening. Stale forms cannot submit, overlapping actions on a recommendation are suppressed, dismissed dialogs remain dismissed, and acknowledged saves with failed refreshes remain recoverable through Approvals. Owned-container event binding avoids duplicate recommendation handlers. The real-Mongo review-concurrency verifier now checks database ownership before model initialization and always disconnects during cleanup. Local tests and synthetic browser checks cover these bounded changes; other mutation/detail flows, live provider/ngrok/HAI acceptance, signed clean-machine Windows installation, and the full production goal remain open.

2.3.61 connects confirmed backend credential rejection to the browser's shared signed-out reset. Expiry, revocation elsewhere, and disabled users are detected on the next protected request, without added polling. Database outages stay retryable, stale responses cannot restore ended-session data, and concurrent single-use invitation success is retained. Other mutation/detail forms, same-session workspace transitions, provider/cloud acceptance, clean-machine Windows acceptance, and the full production goal remain open.

2.3.60 fixes session-list/revocation context handling and two backend authority gaps: explicit invalid credentials cannot fall back to local-owner access, and database credentials cannot administer another workspace merely because the request is local. Self-revocation uses a persisted non-secret signed-out marker with explicit local re-entry; it does not silently regain local authority on Refresh. Real HTTP/Mongo verification covers privileged foreign session/API-token denial, revocation persistence, audits, and subsequent rejection under both access policies.

2.3.59 extends the shared context reset to invitation acceptance and completed workspace deletion. These paths clear previous cached data and permissions, separate storage failures from committed server outcomes, and protect newer workspace/session identities. Closing a submitted invitation or closing/reopening a submitted deletion no longer discards the committed result for an unchanged identity. Focused composed tests and a synthetic browser acceptance journey cover the bounded frontend behavior. Other mutation/detail forms, session revocation, interrupted-onboarding recovery, live account acceptance, and the overall production goal remain partial or external.

2.3.58 fixes the selector refresh gap after the workspace module has loaded, using a shared bounded catalog reader without hidden administration scans. The three forecast forms now enforce original workspace/session/generation ownership, prevent duplicate submissions, discard stale completions, and distinguish an acknowledged save from a failed forecast refresh. Pending scenario results coordinate with ordinary forecast reads. Synthetic browser and composed deferred-response checks cover these paths. Other mutation/detail forms and invitation/session transitions remain unverified; cross-user isolation and the overall production goal stay partial.

2.3.57 extends workspace/session/generation and latest-request ownership to ten primary dashboard GET loaders. Workspace adoption clears stored and rendered dashboard evidence, aborts pending connector/enhancement reads, cancels debounced connector search, and reuses cached modules. Delayed approvals loading renders the current workspace without stealing navigation focus; enhancement-area options reset between workspaces. The 62 new regressions and a synthetic browser report-switching journey verify these bounded paths. Already-open forms, scenario submissions, detail reads, invitation/session transitions, and the selector's catalog-refresh usability gap remain open; cross-user isolation stays partial.
...
2.3.53 preserves approval and worker-response evidence after uncertain database writes. Exact primary readback can resume a confirmed decision; otherwise the request reports uncertainty without destructive cleanup. Pending responses cannot drive outcome/accountability results, and worker webhooks reserve delivery identity before matching so a retry cannot attach one message to adjacent work. Synthetic MongoDB commit-then-error and reconnect checks do not close live outage/failover, operator reconciliation, or hosted acceptance gates.

2.3.52 gives initialization one owner and prevents startup from advancing after shutdown is requested. Cleanup waits within the configured grace for the current acquisition, including pending database, migration, synchronization, maintenance, and tunnel work, before draining components. A timeout remains a failed shutdown, and a late acquisition is cleaned rather than becoming a serving runtime. This is local lifecycle evidence, not live-provider interruption or hosted recovery acceptance.

2.3.51 prevents Winston's independent fatal-error exit from interrupting the installed runtime cleanup handler, while preserving the standalone-command fallback. Fatal errors during signal shutdown retain a failure exit status without duplicating cleanup. Windows package verification pins verified process identities, detects newly observed descendants during close, and cannot mistake a reused numeric PID for an owned process. These corrections do not establish the cause of the historical intermittent normal-close failure.
...
| 046 Cross-user isolation | Partial | Database credentials stay workspace-bound on localhost; privileged foreign session/API-token revocations are rejected in real HTTP/Mongo checks. Administration/report downloads, primary GET readers, selector, forecast forms, invitations, deletion, session list/revocation transitions, matching same-browser session invalidation, recommendation-evidence reads, work-graph detail reads, worker-response binding setup, and connector account-selection dialogs have bounded guards. Other forms/detail reads, idle cross-device revocation notification, and interrupted-onboarding recovery remain open. |
...
| 053 Backup/restore | Partial | Native synthetic dump/restore rehearsal verifies raw BSON, 42 collections/444 indexes, approval/reconciliation evidence, encryption-key dependence, HAI reads, and guarded cleanup. Actual encrypted production backups, secrets recovery, representative volume, consistency, and recovery targets remain external acceptance gates. |
...
| 064 Threat model/security review | Partial | `SECURITY.md`, a historical technical audit, and adversarial tests exist. A current repository-wide deep scan could not start because workers require an unavailable managed filesystem permission profile; no current no-findings claim is supported. |
