...
| 041 Frontend/component tests | Partial | Static/UI assertions plus isolated jsdom coverage verify drafts/presets, help, connector and workspace rendering/action delegation, complete renderer localization, guarded callback presence, demand-load boundaries, and per-asset cache fingerprints; broader approval component coverage remains backlog. |
...
| 050 Responsive/browser compatibility | Partial | Help plus the demand-loaded connector and workspace renderers pass desktop/480x844 containment with no control overflow; packaged Windows 150% scaling passes, while the clean-VM 125%/200% matrix remains pending. |
...
| 057 Dutch/English readiness | Partial | A persistent, tested English/Dutch catalog covers the static shell, setup, command palette, contextual help/search, primary mission control, complete connector marketplace chrome, and the workspace administration renderer including identity, policy, rollout, integrity, retention, and safety-history controls. Provider/user/audit evidence is intentionally preserved verbatim; remaining approval and consequential workspace modal/form chrome still needs explicit semantic catalogs. |
...
| 092 Fresh-clone run | Implemented | Exact 2.3.15 source `2fa0a91691c74dc68c552e56f6ce08227fbbe826` passed GitHub run `31316707100`; both Node 24 quality and Windows installer jobs completed successfully, and artifact `9038991309` was independently downloaded and verified. |
...
| 114 Maintenance/refactor review | Partial | Form persistence, contextual help, localization, connector rendering, and workspace rendering are isolated behind bounded modules. Connector/workspace modules are demand-loaded with retry recovery, guarded mutations remain in the controller, and the eager app is down to 5,737 lines; approval modularization remains backlog. |
