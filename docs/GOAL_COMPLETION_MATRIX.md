...
The current working-tree follow-up broadcasts confirmed session invalidation to matching tabs in the same browser using a SHA-256 fingerprint rather than the session credential. Focused UI tests cover credential rejection, cross-tab cache clearing, unrelated-session isolation, and completed workspace deletion. This does not notify idle sessions on separate devices. The full local gate passes 158 Jest suites/1,534 tests, 180 route-authorization contracts, and all five recommendation scenarios.

The 2.3.72 local gate passed lint, 180 route-authorization contracts (174 guarded, six explicitly public, no issues), 158 Jest suites/1,532 tests, and five recommendation scenarios. The owned-Mongo verifier reported zero real provider calls. Local tests do not establish live-account permissions, deployments, or a signed Windows release.
...
| 046 Cross-user isolation | Partial | Database credentials stay workspace-bound on localhost; privileged foreign session/API-token revocations are rejected in real HTTP/Mongo checks. Administration/report downloads, primary GET readers, selector, forecast forms, invitations, deletion, session list/revocation transitions, and matching same-browser session invalidation have bounded guards. Other forms, detail reads, idle cross-device revocation notification, and interrupted-onboarding recovery remain open. |
