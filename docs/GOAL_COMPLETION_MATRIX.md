...
| 046 Cross-user isolation | Partial | Backend workspace identity and authorization regression coverage; administration/report downloads and ten primary dashboard GET readers guarded. Open forms, scenario submissions, detail reads, invitation/session transitions, and the workspace-selector refresh path still need full context-transition verification. |
