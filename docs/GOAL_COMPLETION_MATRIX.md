...
2.3.65 adds persisted internal-effect recovery to new manual Trello reconciliations. Stable follow-up/audit identities and guarded intervention updates support exact-evidence endpoint retries and bounded retries through the existing follow-up worker. Pending work remains visible, responses received before follow-up insertion are checked during recovery, and cancelled/inconsistent links remain fail-closed. Real MongoDB fault injection and synthetic browser interactions verify this bounded path, not real provider acceptance or process-kill/failover behavior. Recovery of historical records and every normal provider-execution/worker-response crash path, late contradictory provider evidence, other UI contexts, live provider/ngrok/HAI acceptance, signed clean-machine Windows installation, and the full production/resource objective remain incomplete.
