...
- Notification-policy create/edit, activation, external test delivery, and inline pause controls did not bind asynchronous writes and completion notices to their opening UI owner. Six initial red tests reproduced stale submissions and late dialog closes. Forms now capture workspace/session and modal ownership before submit; requests already in flight cannot close a replacement dialog, refresh a different workspace, or cover a newer modal with a success/error notice. Inline pause retains single-flight and retry behavior while suppressing stale notices. Eight focused regressions pass; policy activation and external test delivery remain explicit user-confirmed operations and server authorization is unchanged.
- Connector account-selection and inbound worker-response forms did not retain ownership after opening: an in-flight save could close a newer dialog, and a preserved form could submit after its workspace/session changed. A deferred-save test reproduced the close-before-fix. Both forms now check workspace/session/account/request/modal ownership before writes and after asynchronous results; connector-list refresh and notices are guarded, and delayed member/card searches cannot populate an obsolete form. The focused connector suites pass 44 tests; the full 160-suite gate passes 1,581 tests. No provider writes were made.
- The full dependency audit found the high-severity [GHSA-2883-xcg3-v3hh](https://github.com/advisories/GHSA-2883-xcg3-v3hh) in development/build-tool transitive `js-yaml` 3.15.1 and 4.3.1 copies (Jest/Istanbul and ESLint/electron-builder). The production dependency audit was clean and no application YAML import was found. The lockfile now uses patched 3.15.2 and 4.3.2; full and production audits report zero vulnerabilities. The current comprehensive source scan remains unverified because its managed worker environment is unavailable.
- Removed the remaining eager Enhancements rendering path. The new fingerprinted renderer is demand-loaded and retry-safe, owns only presentation and transient filter bindings, and has no API, session, credential, cookie, storage, or provider authority. Authenticated reads start concurrently in the controller; superseded requests abort and stale results cannot render.
- Reused the deterministic recommendation evaluation for the page session instead of recomputing it on every filter change. Initial app-plus-localization transfer fell from 209,451 to 205,661 raw bytes, 47,669 to 46,569 gzip bytes, and 40,324 to 39,522 Brotli bytes; the deferred module is 8,860 raw, 2,494 gzip, and 2,150 Brotli bytes.
